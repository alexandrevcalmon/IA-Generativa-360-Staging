...
// Hook para buscar mentorias visíveis para uma empresa específica
export const useCompanyVisibleMentorships = (companyId?: string) => {
  return useQuery({
    queryKey: ['company-visible-mentorships', companyId],
    queryFn: async () => {
      if (!companyId) return [];
      ...
      const { data, error } = await supabase
        ...
        .or(`is_collective.eq.true,and(is_collective.eq.false,target_company_id.eq.${companyId})`)
        .order('scheduled_at', { ascending: true });

      if (error) {
        console.error('Error fetching company visible mentorships:', error);
        throw error;
      }
      return data as MentorshipSession[];
    },
