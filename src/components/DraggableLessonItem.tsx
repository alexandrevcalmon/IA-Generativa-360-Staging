...
import { Edit, Trash2, MoreVertical, Play, Clock, Gift, GripVertical } from "lucide-react";
...
export const DraggableLessonItem = ({ lesson, index, onEdit }: DraggableLessonItemProps) => {
    ...
    <Draggable draggableId={lesson.id} index={index}>
      {(provided, snapshot) => (
        <Card 
          ...
          className={`hover:shadow-sm transition-shadow ${snapshot.isDragging ? 'shadow-lg' : ''}`}
        >
