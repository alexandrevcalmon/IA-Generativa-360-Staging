    ...
    <Card className={cn(getCardClass(), className)}>
      {(title || description || headerContent) && (
        <CardHeader className={cn("pb-2", headerClassName)}>
          <div className="flex items-center justify-between">
            <div>
              {title && <CardTitle className={variant === 'default' ? "section-title" : "text-white font-semibold text-xl"}>{title}</CardTitle>}
              {description && <CardDescription className={variant === 'default' ? "section-description" : "text-white/80 text-sm"}>{description}</CardDescription>}
            </div>
            {headerContent && <div>{headerContent}</div>}
          </div>
        </CardHeader>
