...
serve(async (req) => {
  console.log('[create-collaborator-v2] Function invoked. Method:', req.method);
  if (req.method === 'OPTIONS') {
    console.log('[create-collaborator-v2] Handling OPTIONS request.');
  ...
  try {
    ...
    console.error('[create-collaborator-v2] Error parsing request body:', e.message);
  ...
  console.log('[create-collaborator-v2] Request body parsed:', { 
    company_id, 
    name, 
    email, 
    phone, 
  ...
  if (!company_id || !name || !email || !position) {
    console.error('[create-collaborator-v2] Missing required parameters');
  ...
  if (!emailRegex.test(email)) {
    console.error('[create-collaborator-v2] Invalid email format:', email);
  ...
  try {
    ...
    console.log('[create-collaborator-v2] Supabase admin client initialized.');

    // Authorization: Check if the calling user is a producer
    const authHeader = req.headers.get('Authorization');
    if (!authHeader) {
      console.error('[create-collaborator-v2] Authorization header missing.');
    ...
    console.log('[create-collaborator-v2] Fetching calling user.');
    const { data: { user: callingUser }, error: callingUserError } = await supabaseUserClient.auth.getUser();

    if (callingUserError || !callingUser) {
      console.error('[create-collaborator-v2] Error fetching calling user:', callingUserError?.message);
      return new Response(JSON.stringify({ error: 'Unauthorized: Could not verify calling user' }), { 
        status: 401, headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
      });
    }
    console.log(`[create-collaborator-v2] Calling user: ${callingUser.id}, email: ${callingUser.email}`);

    // Verificar se o usuário é um producer ou company
    console.log(`[create-collaborator-v2] Verifying if calling user ${callingUser.id} is a producer or company.`);
    ...
    if (profileError || (profile?.role !== 'producer' && profile?.role !== 'company')) {
      console.error(`[create-collaborator-v2] Calling user ${callingUser.id} is not a producer or company. Profile role: ${profile?.role}`);
      return new Response(JSON.stringify({ error: 'Forbidden: Calling user is not a producer or company' }), { 
        status: 403, headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
      });
    }
    console.log(`[create-collaborator-v2] Calling user ${callingUser.id} confirmed as ${profile.role}.`);
    ...
    if (profile.role === 'company') {
      ...
      if (userCompanyError || !userCompany) {
        console.error(`[create-collaborator-v2] Company user ${callingUser.id} does not have associated company`);
      ...
      if (userCompany.id !== company_id) {
        console.error(`[create-collaborator-v2] Company user ${callingUser.id} trying to add collaborator to different company`);
        return new Response(JSON.stringify({ error: 'Company users can only add collaborators to their own company' }), { 
          status: 403, headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
        });
      }

      console.log(`[create-collaborator-v2] Company user ${callingUser.id} authorized to add collaborator to company ${company_id}`);
    }

    // Verificar limite de assentos da empresa
    console.log(`[create-collaborator-v2] Checking company seat limit for company: ${company_id}`);
    ...
    if (companyError || !company) {
      console.error('[create-collaborator-v2] Error fetching company:', companyError?.message);
    ...
    if (countError) {
      console.error('[create-collaborator-v2] Error counting active collaborators:', countError.message);
    ...
    if (currentActive >= maxCollaborators) {
      console.error(`[create-collaborator-v2] Seat limit reached: ${currentActive}/${maxCollaborators}`);
      return new Response(JSON.stringify({ 
    ...
    console.log(`[create-collaborator-v2] Checking for existing collaborator: ${email}`);
    ...
    if (existingCompanyUserError) {
      console.error(`[create-collaborator-v2] Error checking existing company_users:`, existingCompanyUserError.message);
    ...
    if (existingCompanyUser) {
      console.log(`[create-collaborator-v2] Found existing company_user: ${existingCompanyUser.auth_user_id}`);
      if (existingCompanyUser.company_id === company_id) {
        if (existingCompanyUser.is_active) {
          console.warn(`[create-collaborator-v2] User ${email} is already an active collaborator.`);
          ...
          console.log(`[create-collaborator-v2] Reactivating inactive collaborator: ${email}`);
          ...
          if (reactivateError) {
            console.error(`[create-collaborator-v2] Error reactivating collaborator:`, reactivateError.message);
        ...
        console.warn(`[create-collaborator-v2] User ${email} is a collaborator in a different company.`);
    ...
    console.log(`[create-collaborator-v2] Checking for existing auth user: ${email}`);
    const { data: { users: userList }, error: listUsersError } = await supabaseAdmin.auth.admin.listUsers();
    if (listUsersError) {
      console.error(`[create-collaborator-v2] Error listing users:`, listUsersError.message);
    ...
    if (existingAuthUser) {
      ...
      console.log(`[create-collaborator-v2] Auth user already exists: ${authUserId}`);
      ...
      if (updateMetaError) {
        console.error(`[create-collaborator-v2] Error updating metadata:`, updateMetaError.message);
      }
    } else {
      // Criar novo usuário via inviteUserByEmail
      console.log(`[create-collaborator-v2] Creating new auth user via invite: ${email}`);
      const redirectTo = Deno.env.get('SUPABASE_ACTIVATION_REDIRECT_URL') || Deno.env.get('FRONTEND_URL') + '/activate-account';
      ...
      if (inviteError || !newAuthUserData?.user) {
        console.error(`[create-collaborator-v2] Error inviting user:`, inviteError?.message);
      ...
      isNewAuthUser = true;
      console.log(`[create-collaborator-v2] New auth user invited successfully: ${authUserId}`);
    }

    // Inserir registro na tabela company_users (apenas campos básicos)
    console.log(`[create-collaborator-v2] Inserting company_users record for: ${authUserId}`);
    ...
    if (companyUserInsertError) {
      console.error(`[create-collaborator-v2] Error inserting company_users:`, companyUserInsertError.message);
      if (isNewAuthUser) {
        console.warn(`[create-collaborator-v2] Deleting orphaned auth user: ${authUserId}`);
    ...
    // Upsert profile
    console.log(`[create-collaborator-v2] Upserting profile for: ${authUserId}`);
    ...
    if (upsertProfileError) {
      console.error(`[create-collaborator-v2] Error upserting profile:`, upsertProfileError.message);
    }

    console.log(`[create-collaborator-v2] Process completed successfully for: ${email}`);
    return new Response(JSON.stringify({ 
    ...
    console.error('[create-collaborator-v2] Unexpected error:', error.message);
